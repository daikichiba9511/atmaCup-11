...
path = "../output/exp002/exp002_fold0epoch=05.ckpt"
data_module = MyDataModule(Config())
ssl_model = SimSiam.load_from_checkpoint(
    ...
    batch_size=256,
    optimizer="adam",
    arch="resnet18", # ここで学習したのと同じじゃないとエラーが起きる（simsiam.online_network.encodeのネットワーク構造が変わるから）
    num_workers=4,
    max_epochs=200,
...
model = ssl_model.online_network.encoder
...
model2 = torch.nn.Sequential(
    layer for layer in model.children() if 
)

model.fc = torch.nn.Linear(in_features=512, out_features=1)

pl_model = MyLightningModule(Config())
...
model2 = torch.nn.Sequential(*model_)
model2.fc = torch.nn.Linear(in_features=512, out_features=1)
# -
...
# +
import sys
sys.path.append("/content/drive/MyDrive/atma/atma_11")
from src import utils
os.chdir("/content/drive/MyDrive/atma/atma_11")
# !export WANDB_NOTEBOOK_NAME="eda002"
config = Config()
fold=config.fold
secret_cfg = utils.get_config()
runfilepath = Path("exp002.py")
