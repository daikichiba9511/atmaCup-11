...
* batch size 320
* not used best model => after all epochs, model is saved
Reference
...
class Config:
    ...
    optim = OptimConfig(
        optimizer=OptimizerCfg(
            ...
            # params={"lr": 1e-3},
            name="MADGRAD",
            params={"lr": 1e-3, "eps": 1e-6, "weight_decay": 5e-6},
        ),
        scheduler=SchedulerCfg(
...
def train(
    ...
    trainer = Trainer(
        ...
        limit_val_batches=0.34 if config.debug else 1.0,
        callbacks=None,
        logger=loggers,
    )
    ...
    ssl_model = SimSiam(
        ...
        num_samples=len(pd.read_csv(config.train_csv_path)),
    ...
    trainer.save_checkpoint(f"./output/{expname}/{expname}_fold{fold}epoch={config.epochs}.ckpt")
